...
export default defineRailway(() => {
  ...
  const recordings = volume("recordings-data", { sizeMB: 5000 });

  const mediamtx = service("mediamtx", {
    source: github(REPO, { branch: "main" }),
    volumeMounts: { "/recordings": recordings },
    env: {
      RAILWAY_DOCKERFILE_PATH: "Dockerfile.mediamtx",
    },
  });
  ...
  const worker = service("cohi-worker", {
    ...
    env: {
      ...
      INTERNAL_SERVICE_TOKEN: "${{cohi-api.INTERNAL_SERVICE_TOKEN}}",
